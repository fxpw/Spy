			...
			args = {
				...
				SelectWarnRace = {
					...
					values = function()
						...
						elseif Spy.EnemyFactionName == "Horde" then
...
function Spy:OnInitialize()
	...
	Spy.FactionName = UnitFactionGroup("player")
...
function Spy:CombatLogEvent(info, timestamp, event, srcGUID, srcName, srcFlags, dstGUID, dstName, dstFlags, ...)
	if Spy.EnabledInZone then
...
--		local petName = UnitName("pet");
		...
		if bit.band(srcFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE and srcGUID and srcName and not SpyPerCharDB.IgnoreData[srcName] then
			...
			if srcType == 0 or srcType == 8 then
				...
				if not playerData or playerData.isGuess then
					learnt, playerData = Spy:ParseUnitAbility(true, event, srcName, class, race, arg9, arg10)
				end
				if not learnt then
				...
				if detected then
					...
					if event == "SPELL_AURA_APPLIED" and (arg10 == L["Stealth"]) then
		...
		if bit.band(dstFlags, COMBATLOG_OBJECT_REACTION_HOSTILE) == COMBATLOG_OBJECT_REACTION_HOSTILE and dstGUID and dstName and not SpyPerCharDB.IgnoreData[dstName] then
			local srcType = bit.band(tonumber("0x"..strsub(dstGUID, 3, 5)), 0x00F)
			if srcType == 0 or srcType == 8 then
				...
				if not playerData or playerData.isGuess then
					learnt, playerData = Spy:ParseUnitAbility(false, event, dstName, class, race, arg9, arg10)
				end
				if not learnt then
		...
		if combatEvent[event] then
			local overkill = 0; --PetKill code end
